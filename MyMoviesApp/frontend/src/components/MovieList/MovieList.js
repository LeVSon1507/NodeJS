...
function MovieList({ apiEndpoint, isOriginal, genre }) {
    ...
    const { results, isLoading } = useFetch(apiEndpoint, `page=${page}&genre=${genre}`, 'GET')
    ...
    return (
        <div className="movieList">
            ...
            <div className="btn-next-prev">
                {(page > 1) && <button className="btn-page" onClick={handlePrevPage}>Prev</button>}
                {(page <= totalPage - 1) && <button className="btn-page" onClick={handleNextPage}>Next</button>}
            </div>
